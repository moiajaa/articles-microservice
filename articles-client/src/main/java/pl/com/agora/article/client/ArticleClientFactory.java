...
public class ArticleClientFactory {
    ...
    public ArticleClient createSimpleArticleClient(String host, int port) throws Exception {
        return new RestfulClientProxyBuilder().withHost(host).withPort(port).withErrorClass(ArticleError.class).withDefaultExceptionClass(ArticleClientException.class).build(ArticleClient.class);        
    }
    
    public ArticleClient createCustomArticleClient(String host, int port, ProxyConfiguration config) throws Exception {
        return new RestfulClientProxyBuilder().withHost(host).withPort(port).withConfig(config).withErrorClass(ArticleError.class).withDefaultExceptionClass(ArticleClientException.class).build(ArticleClient.class);
    }
}
